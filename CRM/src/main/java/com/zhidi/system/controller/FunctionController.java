...
//		}
	...
	public String toUpdateFunction(Integer funcId,Model model){
		Function func=funcService.findById(funcId);
		List<Function> parentFunctions=funcService.listParentFunction();
		model.addAttribute("func", func);
		model.addAttribute("parentFunctions", parentFunctions);
		return "system/function/updateFunction";
	}
	...
	public JsonResult updateFunction(Function function){
		if(function==null){
			return JsonResult.buildFailureResult(-200, "修改权限信息失败");
		}
		Function func=funcService.findById(function.getFuncId());
		function.setCreateTime(func.getCreateTime());
		function.setUpdateTime(new Date());
		Integer rows=funcService.update(function);
		if (rows > 0) {
			return JsonResult.buildSuccessResult("修改权限信息成功！");
		} else {
			return JsonResult.buildFailureResult(-200, "修改权限信息失败！");
		}
	}
